...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Render Progress Bar (same as PPK)
    function renderProposalProgressJS(tahapSekarang, status, forMobile = false) {
        ...
        if (posisiSekarang === -1 && isLPJ) posisiSekarang = 5;
    ...
    async function fetchData() {
        ...
        const url = `/docutrack/public/direktur/monitoring/data?page=${currentPage}&status=${currentStatus}&jurusan=${encodeURIComponent(currentJurusan)}&search=${encodeURIComponent(currentSearch)}`;
