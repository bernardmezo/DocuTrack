...
<main class="main-content font-poppins p-7 -mt-8 md:-mt-20 max-w-7xl mx-auto w-full">
    ...
    ?>
                            <tr class='wadir-row <?php echo $row_class; ?> hover:bg-gray-100 transition-colors' 
        ...
        <div id="pagination-wadir" class="flex flex-col sm:flex-row justify-between items-center px-6 py-4 border-t border-gray-200 gap-4">
            </div>
    </section>
</main>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    class WadirTableManager {
        ...
        render() {
            ...
            this.tbody.innerHTML = pageData.map((item, i) => {
                ...
                <tr class="hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-0">
                    ...
                    <td class="px-6 py-5 whitespace-nowrap text-sm font-medium">
                        <a href="/docutrack/public/wadir/telaah/show/${item.id}" class="bg-blue-600 text-white px-4 py-2 rounded-md text-xs font-medium hover:bg-blue-700 transition-colors inline-flex items-center gap-2">
        ...
        renderPagination(totalPages) {
            ...
            let btns = '';
            // Prev Button
            btns += `<button onclick="wadirTable.goToPage(${this.currentPage - 1})" ${this.currentPage === 1 ? 'disabled' : ''} class="px-3 py-2 rounded-md text-sm font-medium bg-white text-gray-700 border border-gray-300 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"><i class="fas fa-chevron-left"></i></button>`;
            ...
            for (let i = 1; i <= totalPages; i++) {
                ...
                btns += `<button onclick="wadirTable.goToPage(${i})" class="px-3 py-2 rounded-md text-sm font-medium border transition-colors ${active}">${i}</button>`;
            }

            // Next Button
            btns += `<button onclick="wadirTable.goToPage(${this.currentPage + 1})" ${this.currentPage === totalPages ? 'disabled' : ''} class="px-3 py-2 rounded-md text-sm font-medium bg-white text-gray-700 border border-gray-300 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"><i class="fas fa-chevron-right"></i></button>`;
