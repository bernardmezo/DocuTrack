    ...
    <section id="stage-list" class="stage-content bg-white p-4 md:p-7 rounded-2xl shadow-lg overflow-hidden mb-8">
        ...
        <div class="mb-6 flex justify-end"> 
            <div class="relative w-full md:w-80">
                ...
                <input type="text" id="search-telaah-input" placeholder="Cari Nama Kegiatan..."
        ...
        <div class="overflow-x-auto max-h-96 border border-gray-100 rounded-lg">
            <table class="w-full min-w-[700px]">
                ...
                <tbody class="divide-y divide-gray-100">
                    <?php
                        if (!empty($list_usulan)):
                            ...
                            foreach ($list_usulan as $item):
                                ...
                                // --- LOGIKA UI/UX 'BRILLIANT' ---
                                $status_text = htmlspecialchars($item['status'] ?? 'N/A');
                                $status_class = ''; // Untuk badge
                                $icon_class = '';   // Untuk ikon di badge
                                $row_class = 'bg-white';    // Default

                                if (strtolower($status_text) === 'telah direvisi') {
                                    // Prioritas 1: Highlight Ungu
                                    $status_class = 'text-purple-700 bg-purple-100';
                                    $icon_class = 'fas fa-sync-alt'; 
                                    $row_class = 'bg-purple-50 font-medium'; // Baris di-highlight
                                } else {
                                    // Prioritas 2: Highlight Abu-abu
                                    $status_class = 'text-gray-700 bg-gray-100';
                                    $icon_class = 'fas fa-hourglass-half';
                                    $row_class = 'bg-gray-50'; // Baris di-highlight
                                }
                                // --- AKHIR LOGIKA UI/UX ---
                    ?>
                                <tr class='<?php echo $row_class; ?> hover:bg-gray-100 transition-colors'>
                                    ...
                                    <td class='px-4 py-3 md:px-6 md:py-5 whitespace-nowrap text-sm font-medium'>
                                        <div class='flex gap-2 items-center'>
                                            <a href="/docutrack/public/verifikator/telaah/show/<?php echo $item['id'] ?? ''; ?>?ref=telaah_list" 
                    ...
                    ?>
                        <tr>
                            <td colspan="5" class="text-center py-10 text-gray-500 italic">Antrian telaah Anda kosong. Kerja bagus!</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        const searchInput = document.getElementById('search-telaah-input');
        ...
        function filterTable() {
            ...
            if (emptyRow) {
                emptyRow.parentElement.style.display = hasVisibleRows ? 'none' : (tableRows.length > 1 ? 'none' : '');
            }
        }
