    ...
    <section class="grid grid-cols-2 lg:grid-cols-3 gap-3 mb-5">
        ...
        <div class="relative group p-4 rounded-xl shadow-md overflow-hidden text-white
            ...
            <div class="relative z-10 flex flex-col gap-1">
                ...
                <p class="text-xs md:text-sm font-medium opacity-90">Disetujui Verifikator</p>
                <div class="mt-2 text-right opacity-70">
    ...
    <section class="bg-white rounded-xl shadow-lg overflow-hidden mb-5 flex flex-col">
        <div class="p-4 border-b border-gray-200">
            ...
            <div class="flex flex-col sm:flex-row gap-2 sm:gap-3 mb-3">
                <div class="relative flex-1">
                    <select id="filter-status" 
                        ...
                        <option value="disetujui verifikator" style="color: #374151 !important;">Disetujui Verifikator</option>
                    </select>
            ...
            <div class="mt-3 sm:mt-4 relative">
                <input type="text" id="search-ppk-input" placeholder="Cari nama kegiatan, pengusul, atau NIM..." class="w-full pl-9 sm:pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200">
        ...
        <div class="hidden md:block overflow-x-auto">
            <div class="overflow-y-auto" style="max-height: 500px;">
                <table class="min-w-full" id="table-ppk">
                    ...
                    <tbody id="ppk-table-body" class="divide-y divide-gray-100 bg-white"></tbody>
                </table>
            </div>
        </div>
        ...
        <div class="md:hidden overflow-y-auto" style="max-height: 500px;">
            <div id="mobile-ppk-list" class="p-3 space-y-3"></div>
        </div>

        <!-- Pagination -->
        <div class="p-3 border-t border-gray-200 bg-gray-50">
            <div class="flex flex-col gap-3">
                <div id="pagination-ppk" class="flex gap-1 flex-wrap justify-center"></div>
                <div class="text-xs text-gray-600 text-center">
                    Menampilkan <span id="showing-ppk" class="font-semibold text-gray-800">0</span> dari <span id="total-ppk" class="font-semibold text-gray-800">0</span> data
                </div>
            </div>
        </div>
    </section>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    class PPKTableManager {
        constructor(data) {
            ...
            this.mobileList = document.getElementById('mobile-ppk-list');
            this.paginationContainer = document.getElementById('pagination-ppk');
            this.showingSpan = document.getElementById('showing-ppk');
            this.totalSpan = document.getElementById('total-ppk');
        ...
        render() {
            ...
            if(this.tbody) {
                this.tbody.innerHTML = pageData.map((item, i) => {
                    ...
                    const statusClass = statusLower === 'disetujui verifikator' ? 'text-green-700 bg-green-100' : 'text-gray-600 bg-gray-100';
                    const iconClass = statusLower === 'disetujui verifikator' ? 'fa-check-circle' : 'fa-hourglass-half';
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-5 whitespace-nowrap text-sm font-medium">
                            <a href="/docutrack/public/ppk/telaah/show/${item.id}?ref=dashboard" class="bg-blue-600 text-white px-4 py-2 rounded-md text-xs font-medium hover:bg-blue-700 transition-colors inline-flex items-center gap-2">
                ...
                this.mobileList.innerHTML = pageData.map((item, i) => {
                    ...
                    <div class="mobile-card">
                        ...
                        <div class="mobile-card-actions">
                            <a href="/docutrack/public/ppk/telaah/show/${item.id}?ref=dashboard" class="mobile-card-btn">
        ...
        renderPagination(totalPages) {
            ...
            let btns = '';
            // Prev Button
            btns += `<button onclick="ppkTable.goToPage(${this.currentPage - 1})" ${this.currentPage === 1 ? 'disabled' : ''} class="pagination-buttons"><i class="fas fa-chevron-left"></i></button>`;
            ...
            if (startPage > 1) {
                btns += `<button onclick="ppkTable.goToPage(1)" class="pagination-buttons">1</button>`;
                if (startPage > 2) btns += `<span class="px-2 text-gray-400">...</span>`;
            }

            for (let i = startPage; i <= endPage; i++) {
                const active = i === this.currentPage ? 'active' : '';
                btns += `<button onclick="ppkTable.goToPage(${i})" class="pagination-buttons ${active}">${i}</button>`;
            }

            if (endPage < totalPages) {
                if (endPage < totalPages - 1) btns += `<span class="px-2 text-gray-400">...</span>`;
                btns += `<button onclick="ppkTable.goToPage(${totalPages})" class="pagination-buttons">${totalPages}</button>`;
            }

            // Next Button
            btns += `<button onclick="ppkTable.goToPage(${this.currentPage + 1})" ${this.currentPage === totalPages ? 'disabled' : ''} class="pagination-buttons"><i class="fas fa-chevron-right"></i></button>`;
