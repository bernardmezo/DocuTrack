...
$current = $active_page ?? ''; // $active_page dikirim dari Controller
?>
<!DOCTYPE html>
<html lang="id">
<head>
        ...
        <div class="top-section bg-gradient-to-l from-[#17A18A] via-[#006A9A] to-[#114177] p-6 pb-4 md:pb-20 text-white shadow-lg"> 
             <header class="flex justify-between items-center pb-5 border-b border-white/20 max-w-7xl mx-auto">
                 <div class="flex items-center gap-4 md:gap-10">
                     <div class="w-40 md:w-auto">
                        <a href="/docutrack/public/verifikator/dashboard"> <!-- Link logo ke dashboard -->
                         ...
                         <ul class="flex gap-4">
                             <li>
                                 <a href="/docutrack/public/verifikator/dashboard" 
                                    class="flex items-center gap-2 px-4 py-2 rounded-full transition-colors <?php echo isActive($current, '/verifikator/dashboard'); ?>">
                                     <i class="fas fa-th-large text-sm"></i> Dashboard
                                 </a>
                             </li>
                             <li>
                                 <a href="/docutrack/public/verifikator/pengajuan-telaah" 
                                    class="flex items-center gap-2 px-4 py-2 rounded-full transition-colors <?php echo isActive($current, '/verifikator/pengajuan-telaah'); ?>">
                                     <i class="fas fa-file-import text-sm"></i> Pengajuan Telaah
                                 </a>
                             </li>
                             <li>
                                 <a href="/docutrack/public/verifikator/riwayat-verifikasi" 
                                    class="flex items-center gap-2 px-4 py-2 rounded-full transition-colors <?php echo isActive($current, '/verifikator/riwayat-verifikasi'); ?>">
                    ...
                    <div class="relative text-xl text-gray-200 hover:text-white cursor-pointer transition-colors duration-200">
                     ...
                     <div class="relative">
                          <div id="profile-menu-button" class="flex items-center gap-3 cursor-pointer group">
                               <div class="w-10 h-10 rounded-full bg-cover ring-2 ..." style="background-image: url('https.../text=PY')"></div>
                               <div class="hidden sm:block">
                                    <div class="font-semibold text-sm text-white"><?php echo htmlspecialchars($_SESSION['user_name'] ?? 'Nama User'); ?></div>
                                    <div class="text-xs text-gray-300"><?php echo htmlspecialchars(ucfirst($_SESSION['user_role'] ?? 'Role')); ?></div>
                               </div>
                          </div>
                          <!-- Menu Dropdown -->
            ...
            <div id="mobile-admin-menu" class="md:hidden hidden max-w-7xl mx-auto pt-4 pb-2">
                <div class="px-2 space-y-1">
                    <a href="/docutrack/public/verifikator/dashboard" class="flex items-center gap-2 px-3 py-2 rounded-md text-base font-medium <?php echo isActive($current, '/verifikator/dashboard') ? 'bg-white/10 text-white' : 'text-gray-200 hover:bg-white/10 hover:text-white'; ?>"><i class="fas fa-th-large text-sm w-5 text-center"></i> Dashboard</a>
                    <a href="/docutrack/public/verifikator/pengajuan-telaah" class="flex items-center gap-2 px-3 py-2 rounded-md text-base font-medium <?php echo isActive($current, '/verifikator/pengajuan-telaah') ? 'bg-white/10 text-white' : 'text-gray-200 hover:bg-white/10 hover:text-white'; ?>"><i class="fas fa-file-import text-sm w-5 text-center"></i> Pengajuan Telaah</a>
                    <a href="/docutrack/public/verifikator/riwayat-verifikasi" class="flex items-center gap-2 px-3 py-2 rounded-md text-base font-medium <?php echo isActive($current, '/verifikator/riwayat-verifikasi') ? 'bg-white/10 text-white' : 'text-gray-200 hover:bg-white/10 hover:text-white'; ?>"><i class="fas fa-history text-sm w-5 text-center"></i> Riwayat Verifikasi</a>
                </div>
            </div>
        </div> 
