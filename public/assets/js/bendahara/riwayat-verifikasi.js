...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function isProdiInJurusan(namaProdi, selectedJurusan) {
        ...
        // Cek langsung (jika backend kirim field jurusan)
    ...
    class TableManager {
        ...
        applyFilters() {
            ...
            this.filteredData = this.allData.filter(item => {
                ...
                // 3. Filter Jurusan PNJ
                let matchJurusan = true;
                if (jurusanFilter) {
                    if (item.jurusan === jurusanFilter) {
                        matchJurusan = true;
                    } 
                    else if (item.prodi) {
                        matchJurusan = isProdiInJurusan(item.prodi, jurusanFilter);
                    }
                    else {
                        matchJurusan = false;
                    }
                    ...
                    <tr class='${rowClass} hover:bg-blue-50/50 transition-colors duration-150 cursor-pointer' onclick="window.location.href='/docutrack/public/bendahara/riwayat-verifikasi/show/${item.id}?ref=riwayat-verifikasi'">
                        ...
                        <td class='px-6 py-5 text-sm'>
                            <div class="flex flex-col">
                                <span class="font-semibold text-gray-900 mb-1 hover:text-blue-600">${this.escapeHtml(item.nama)}</span>
